...
- First argument: identifier (used in transitions, steps, profiles).
- Second argument: human-readable display name.
...
- `prompt <ident>` resolves to `prompts/<ident>.md`.
...
- Left side must be a `queue` state.
- Right side must be an `action` state.
...
- `prompt <ident>` resolves to `prompts/<ident>.md`.
- Step queue/action references resolve to declared queue/action states.
